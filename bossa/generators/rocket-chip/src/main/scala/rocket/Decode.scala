...
object Simplify
{
  ...
  def getCover(implicants: Seq[Term], minterms: Seq[Term], bits: Int) = {
    if (minterms.nonEmpty) {
      ...
      val visited = collection.mutable.Set[Seq[Term]]()
      cover.tail.foldLeft(cover.head.map(Set(_)).toSet) { (c0, c1) =>
        if (visited.add(c1)) c0.flatMap(a => c1.map(a + _)) else c0
      }.map(_.toList).reduceLeft((a, b) => if (cheaper(a, b, bits)) a else b)
    } else
      Seq[Term]()
  }
