...
class FPResult(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  val u_exception = Bits(width = 7)
  val u_flag = Bits(width = 10)
  val u_c = Bits(width = 1)
}
...
class FPToInt(implicit p: Parameters) extends FPUModule()(p) with ShouldBeRetimed {
  class Output extends Bundle {
    ...
    // mgy 添加输出端口
    val toint_cmp_unicore = Bits(width = xLen)
    val u_exception = Bits(width = 7)
    val u_flag = Bits(width = 10)
    val u_c = Bits(width = 1)
  }
  ...
  when(in.is_unicore) {
    /*Cat(isQNaN, isSNaN, isInf && !sign, isNormal && !sign,
        isSubnormal && !sign, isZero && !sign, isZero && sign,
        isSubnormal && sign, isNormal && sign, isInf && sign)*/
    val in1_type = (floatTypes.map(t => t.classify(maxType.unsafeConvert(in.in1, t))): Seq[UInt])(in.fmt)
    val in2_type = (floatTypes.map(t => t.classify(maxType.unsafeConvert(in.in2, t))): Seq[UInt])(in.fmt)
    val isSubnormal = in1_type(2) || in1_type(5) || in2_type(2) || in2_type(5)
    val overflow_i = 0.U(1.W)
    val inexact = 0.U(1.W)
    val underflow = 0.U(1.W)
    val overflow_f = 0.U(1.W)
    val divzero = 0.U(1.W)
    val invalid_op = (maxType.isSNaN(in.in1) || maxType.isSNaN(in.in2)) && (in.cond_unicore(3,3) === 1.U)
    val isZero = 0.U(1.W)
    val isInf = 0.U(1.W)
    val isRemain = 0.U(1.W)
    io.out.bits.u_exception := Cat(invalid_op, divzero, overflow_f, underflow, inexact, overflow_i, isSubnormal) & in.exc_enabled
    io.out.bits.u_flag := Cat(divzero, isRemain, isSubnormal, overflow_i, inexact, overflow_f, underflow, invalid_op, isInf, isZero)
    io.out.bits.u_c := 0.U
  }
  ...
  // mgy
  val toint_cmp_unicore = Wire(init = store)
  io.out.bits.toint_cmp_unicore := toint_cmp_unicore
  // mgy 定义条件码，没有想到该放在哪个头文件合适，先放在这里叭 
  val Cond_F    = 0.U(4.W) // 假
  val Cond_UN   = 1.U(4.W) // 无序
  val Cond_EQ   = 2.U(4.W) // 相等
  val Cond_UEQ  = 3.U(4.W) // 无序或相等
  val Cond_OLT  = 4.U(4.W) // 有序的小于
  val Cond_ULT  = 5.U(4.W) // 无序或小于
  val Cond_OLE  = 6.U(4.W) // 有序的小于或等于
  val Cond_ULE  = 7.U(4.W) // 无序或小于或等于
  val Cond_SF   = 8.U(4.W) // 传信号的假
  val Cond_NGLE = 9.U(4.W) // 不（大于或小于或等于）
  val Cond_SEQ  = 10.U(4.W) // 传信号的相等
  val Cond_NGL  = 11.U(4.W) // 不（大于或小于）
  val Cond_LT   = 12.U(4.W) // 小于
  val Cond_NGE  = 13.U(4.W) // 不（大于或等于）
  val Cond_LE   = 14.U(4.W) // 小于或等于
  val Cond_NGT  = 15.U(4.W) // 不大于
  ...
  when (in.wflags) { // feq/flt/fle, fcvt
    ...
    // mgy 根据 UniCoreF64 手册得到输出信号
    toint_cmp_unicore :=  Mux(in.cond_unicore === Cond_F,                                      0.U,//假，未实现
                          Mux(in.cond_unicore === Cond_UN  && !dcmp.io.ordered,                1.U,
                          Mux(in.cond_unicore === Cond_EQ  &&  dcmp.io.eq,                     1.U,
                          Mux(in.cond_unicore === Cond_UEQ &&(!dcmp.io.ordered || dcmp.io.eq), 1.U,
                          Mux(in.cond_unicore === Cond_OLT &&  dcmp.io.lt,                     1.U,
                          Mux(in.cond_unicore === Cond_ULT &&(!dcmp.io.ordered || dcmp.io.lt), 1.U,
                          Mux(in.cond_unicore === Cond_OLE &&(!dcmp.io.eq || dcmp.io.lt)     , 1.U,
                          Mux(in.cond_unicore === Cond_ULE && !dcmp.io.gt,                     1.U,
                          Mux(in.cond_unicore === Cond_SF,                                     0.U,//传信号的假，未实现
                          Mux(in.cond_unicore === Cond_NGLE&& !dcmp.io.ordered,                1.U,
                          Mux(in.cond_unicore === Cond_SEQ &&  dcmp.io.eq,                     1.U,//传信号的相等，未实现
                          Mux(in.cond_unicore === Cond_NGL &&(!dcmp.io.ordered || dcmp.io.eq), 1.U,
                          Mux(in.cond_unicore === Cond_LT  &&  dcmp.io.lt,                     1.U,
                          Mux(in.cond_unicore === Cond_NGE &&(!dcmp.io.ordered || dcmp.io.lt), 1.U,
                          Mux(in.cond_unicore === Cond_LE  &&( dcmp.io.lt || dcmp.io.eq),      1.U,
                          Mux(in.cond_unicore === Cond_NGT && !dcmp.io.gt,                     1.U,
                          0.U
                          ))))))))))))))))
        ...
        when (cvtType === i) {
          ...
          when(in.is_unicore) {
            /*Cat(isQNaN, isSNaN, isInf && !sign, isNormal && !sign,
              ...
              isSubnormal && sign, isNormal && sign, isInf && sign)*/
            val in1_type = (floatTypes.map(t => t.classify(maxType.unsafeConvert(in.in1, t))): Seq[UInt])(in.fmt)
            val isSubnormal = in1_type(2) || in1_type(5)
            val overflow_i = 0.U(1.W)
            val inexact = 0.U(1.W)
            val underflow = 0.U(1.W)
            val overflow_f = 0.U(1.W)
            val divzero = 0.U(1.W)
            val invalid_op = maxType.isSNaN(in.in1) || in1_type(0) || in1_type(7)
            val isZero = in1_type(3) || in1_type(4)
            val isInf = in1_type(0) || in1_type(7)
            val isRemain = 0.U(1.W)
            io.out.bits.u_exception := Cat(invalid_op, divzero, overflow_f, underflow, !invalid && conv.io.intExceptionFlags(0), !invalid_op && invalid, isSubnormal) & in.exc_enabled
            io.out.bits.u_flag := Cat(divzero, isRemain, isSubnormal, !invalid_op && invalid, !invalid && conv.io.intExceptionFlags(0), overflow_f, underflow, invalid_op, isInf, isZero)
            io.out.bits.u_c := 0.U
          }
        }
      }
...
class IntToFP(val latency: Int)(implicit p: Parameters) extends FPUModule()(p) with ShouldBeRetimed {
  ...
  when (in.bits.wflags) { // fcvt
    ...
    when(in.bits.is_unicore) {
      /*Cat(isQNaN, isSNaN, isInf && !sign, isNormal && !sign,
        isSubnormal && !sign, isZero && !sign, isZero && sign,
        isSubnormal && sign, isNormal && sign, isInf && sign)*/
      val isSubnormal = 0.U(1.W)
      val overflow_i = 0.U(1.W)
      val inexact = 0.U(1.W)
      val underflow = 0.U(1.W)
      val overflow_f = 0.U(1.W)
      val divzero = 0.U(1.W)
      val invalid_op = 0.U(1.W)
      val isZero = (intValue === 0.U).asUInt
      val isInf = 0.U(1.W)
      val isRemain = 0.U(1.W)
      mux.u_exception := Cat(exc(tag), overflow_i, isSubnormal) & in.bits.exc_enabled
      mux.u_flag := Cat(exc(tag)(3), isRemain, isSubnormal, overflow_i, exc(tag)(0), exc(tag)(2), exc(tag)(1), exc(tag)(4), isInf, isZero)
      mux.u_c := 0.U
    }
  }
...
class FPToFP(val latency: Int)(implicit p: Parameters) extends FPUModule()(p) with ShouldBeRetimed {
  ...
  when(in.bits.is_unicore) { // fabs_u/fneg_u
    /*Cat(isQNaN, isSNaN, isInf && !sign, isNormal && !sign,
        isSubnormal && !sign, isZero && !sign, isZero && sign,
        isSubnormal && sign, isNormal && sign, isInf && sign)*/
    val u_type = (floatTypes.map(t => t.classify(maxType.unsafeConvert(fsgnj, t))): Seq[UInt])(in.bits.fmt)
    val isSubnormal = u_type(2) | u_type(5)
    val overflow_i = 0.U(1.W)
    val inexact = 0.U(1.W)
    val underflow = 0.U(1.W)
    val overflow_f = 0.U(1.W)
    val divzero = 0.U(1.W)
    val invalid_op = maxType.isSNaN(in.bits.in1) || maxType.isSNaN(in.bits.in2)
    val isZero = u_type(3) | u_type(4)
    val isInf = u_type(0) | u_type(7)
    val isRemain = 0.U(1.W)
    fsgnjMux.u_exception := Cat(invalid_op, divzero, overflow_f, underflow, inexact, overflow_i, isSubnormal) & in.bits.exc_enabled
    fsgnjMux.u_flag := Cat(divzero, isRemain, isSubnormal, overflow_i, inexact, overflow_f, underflow, invalid_op, isInf, isZero)
    fsgnjMux.u_c := 0.U
  }
    ...
    if (floatTypes.size > 1) {
      ...
      when(in.bits.is_unicore) {
        /*Cat(isQNaN, isSNaN, isInf && !sign, isNormal && !sign,
            isSubnormal && !sign, isZero && !sign, isZero && sign,
            isSubnormal && sign, isNormal && sign, isInf && sign)*/
        val in1_type = (floatTypes.map(t => t.classify(maxType.unsafeConvert(in.bits.in1, t))): Seq[UInt])(in.bits.fmt)
        val isSubnormal = in1_type(2) || in1_type(5)
        val overflow_i = 0.U(1.W)
        val inexact = 0.U(1.W)
        val underflow = 0.U(1.W)
        val overflow_f = 0.U(1.W)
        val divzero = 0.U(1.W)
        val invalid_op = maxType.isSNaN(in.bits.in1)
        val isZero = in1_type(3) || in1_type(4)
        val isInf = in1_type(0) || in1_type(7)
        val isRemain = 0.U(1.W)
        io.out.bits.u_exception := Cat(invalid_op, mux.exc(3, 0), overflow_i, isSubnormal) & in.bits.exc_enabled
        io.out.bits.u_flag := Cat(divzero, isRemain, isSubnormal, overflow_i, mux.exc(0), mux.exc(2), mux.exc(1), invalid_op, isInf, isZero)
        io.out.bits.u_c := 0.U
      }
    }
  }
...
class FPUFMAPipe(val latency: Int, val t: FType)
  ...
  when(in.is_unicore&&fma.io.validout){
    val u_type = t.classify(res.data)

    val  u_type1 = t.classify(in.in1)
    val  u_type2 = t.classify(in.in2)
    val  u_type3 = t.classify(in.in3)

    val hasSNaN = t.isSNaN(in.in1)|t.isSNaN(in.in2)|t.isSNaN(in.in3)
    
    val isZero = u_type(3) | u_type(4) 
    val isInf  = u_type(0) | u_type(7)  

    val isInf1 = u_type1(0) | u_type1(7)
    val isInf2 = u_type2(0) | u_type2(7)
    val isInf3 = u_type3(0) | u_type3(7)
  
    //The meaning of each domain in exceptionFlags is as follows
    //Cat(io.invalidExc, io.infiniteExc, overflow, underflow, inexact)
    val underflow = fma.io.exceptionFlags(1)
    val overflow_f = fma.io.exceptionFlags(2)
    

    val overflow_i = 0.U(1.W)
    val isSubnormal = u_type1(2) | u_type1(5)|u_type2(2) | u_type2(5) | u_type3(2) | u_type3(5)
    val inexact = fma.io.exceptionFlags(0) && !isSubnormal && !hasSNaN
    val isRemain = 0.U(1.W)
    val divzero = 0.U(1.W)

    //The fmaCmd bit field is b00 
    //iff this instruction is a multiplication or addition instruction
    when(in.fmaCmd===0.U){
      when(in.swap23===1.U){// must be add
        val PInf_NInf = u_type1(0)&u_type3(7)
        val NInf_PInf = u_type1(7)&u_type3(0)
        val add_invalid_op = hasSNaN | PInf_NInf | NInf_PInf
        res.u_exception := Cat(add_invalid_op, divzero, overflow_f, underflow, inexact, overflow_i, isSubnormal) & in.exc_enabled
        res.u_flag := Cat(divzero, isRemain, isSubnormal, overflow_i, inexact, overflow_f, underflow, add_invalid_op, isInf, isZero)
        res.u_c := 0.U
      }.elsewhen(in.swap23===0.U){// must be mul
        
        val isZero1 = u_type1(3) | u_type1(4)
        val isZero2 = u_type2(3) | u_type2(4)
        val zero_inf = isZero1 & isInf2 // 0 x inf
        val inf_zero = isInf1 & isZero2 // inf x 0
        val mul_invalid_op = hasSNaN | zero_inf | inf_zero
        res.u_exception := Cat(mul_invalid_op, divzero, overflow_f, underflow, inexact, overflow_i, isSubnormal) & in.exc_enabled
        res.u_flag := Cat(divzero, isRemain, isSubnormal, overflow_i, inexact, overflow_f, underflow, mul_invalid_op, isInf, isZero)
        res.u_c := 0.U
      }
    }.elsewhen(in.swap23===1.U){// must be sub
        val sub_invalid_op = hasSNaN |(u_type1(0) & u_type3(0)) //Negative infinity minus negative infinity
        res.u_exception := Cat(sub_invalid_op, divzero, overflow_f, underflow, inexact, overflow_i, isSubnormal) & in.exc_enabled
        res.u_flag := Cat(divzero, isRemain, isSubnormal, overflow_i, inexact, overflow_f, underflow, sub_invalid_op, isInf, isZero)
        res.u_c := 0.U
    }
  }
